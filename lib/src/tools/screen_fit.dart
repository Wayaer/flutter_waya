...
class MediaQueryTools {
  ...
  static bool getAccessibleNavigation() {
    return MediaQueryData.fromWindow(window).accessibleNavigation;
  }

  ///alwaysUse24HourFormat → bool 格式化时间时是否使用24小时格式。
  static bool getBoldText() {
    return MediaQueryData.fromWindow(window).boldText;
  }
  ...
  static bool getDisableAnimations() {
    return MediaQueryData.fromWindow(window).disableAnimations;
  }

  ///  hashCode → int 此对象的哈希码
  static int getHashCode() {
    return MediaQueryData.fromWindow(window).hashCode;
  }

  /// invertColors → bool 设备是否反转平台的颜色
  static bool getInvertColors() {
    return MediaQueryData.fromWindow(window).invertColors;
  }
  ...
  static Brightness getPlatformBrightness() {
    return MediaQueryData.fromWindow(window).platformBrightness;
  }
  ...
  static double getTextScaleFactor() {
    return MediaQueryData.fromWindow(window).textScaleFactor;
  }

  /// viewInsets → EdgeInsets 显示器的各个部分完全被系统UI遮挡，通常是设备的键盘
  static EdgeInsets getViewInsets() {
    return MediaQueryData.fromWindow(window).viewInsets;
  }

  /// viewPadding → EdgeInsets 显示器的部分被系统UI部分遮挡，通常由硬件显示“凹槽”或系统状态栏
  static EdgeInsets getViewPadding() {
    return MediaQueryData.fromWindow(window).viewPadding;
  }

  ///手机屏幕的宽分辨率
  static double getWidthPixel() {
    return MediaQueryData.fromWindow(window).size.width * MediaQueryTools.getDevicePixelRatio();
  }

  ///手机屏幕高分辨率
  static double getHeightPixel() {
    return MediaQueryData.fromWindow(window).size.height * MediaQueryTools.getDevicePixelRatio();
  }
